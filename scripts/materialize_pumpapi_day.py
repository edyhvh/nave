#!/usr/bin/env python3
"""Materialize verified hourly normalized rows into one compact Parquet file."""
...
def main() -> int:
    ...
    entries = checkpoint.get("hours", {})
    if any(entries.get(f"{hour:02d}", {}).get("status") not in {"COMPLETE", "COMPLETE_WITH_WARNINGS"} for hour in range(24)):
        raise SystemExit("refusing to materialize a day with incomplete hours")
    writer = None
    batch: list[dict] = []
    rows = 0
    mints: set[str] = set()
    for hour in range(24):
        ...
        for line in path.open(encoding="utf-8"):
            ...
            batch.append({field: event.get(field) for field in FIELDS})
            if event.get("mint"):
    ...
    if writer is not None:
        writer.close()
    print(json.dumps({"rows": rows, "unique_mints": len(mints), "output": str(args.output), "schema_version": "nave-pumpapi-canonical-event-v2"}, sort_keys=True))
    return 0
