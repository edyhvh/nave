...
if OPENBB_OK:
    ...
    try_openbb("#1 TGA (Treasury General Account)",
        lambda: obb.economy.fred_series(symbol="WDTGAL", limit=5))

    # RRP
    try_openbb("#1 RRP (Reverse Repo Facility)",
        lambda: obb.economy.fred_series(symbol="RRPONTSYD", limit=5))

    # Fed Injections / QE
    try_openbb("#2 Fed Balance Sheet (QE proxy)",
        lambda: obb.economy.fred_series(symbol="WALCL", limit=5))

    # Interest Rates
    try_openbb("#3 Fed Funds Rate",
        lambda: obb.economy.fred_series(symbol="FEDFUNDS", limit=5))
else:
...
if OPENBB_OK:
    ...
    try_openbb("#5 VIX (Risk Appetite)",
        lambda: obb.equity.price.historical(symbol="^VIX", limit=5))
...
if OPENBB_OK:
    ...
    try_openbb("#6 US Debt/GDP Ratio",
        lambda: obb.economy.fred_series(symbol="GFDEGDQ188S", limit=5))

    # Fiat Purchasing Power (CPI-based)
    try_openbb("#8 Fiat Purchasing Power (CPI)",
        lambda: obb.economy.fred_series(symbol="CPIAUCSL", limit=5))
...
if code == 200 and data and data.get("data"):
    customs = [r for r in data["data"] if "customs" in r.get("classification_desc", "").lower()]
    if customs:
        row = customs[0]
        record("#7 Tariff Revenue (Treasury MTS)", "PASS",
               f"Customs Duties: ${float(row['current_month_rcpt_outly_amt']):,.0f} (Feb 2026)")
    else:
        record("#7 Tariff Revenue (Treasury MTS)", "PASS", f"{len(data['data'])} rows (customs row not found in sample)")
else:
...
if code == 200 and data:
    cap = data.get("data", {}).get("total_market_cap", {}).get("usd")
    record("#9 Crypto Market Cap (CoinGecko)", "PASS", f"${cap:,.0f}" if cap else "got data")
else:
...
if OPENBB_OK:
    ...
    try_openbb("#10 Capital Flows (Balance of Payments)",
        lambda: obb.economy.fred_series(symbol="BOPBCA", limit=5))

    # GDP Growth
    try_openbb("#11 Economic Growth (Real GDP)",
        lambda: obb.economy.fred_series(symbol="GDPC1", limit=5))

    # PPP - USD/EUR exchange rate via FRED
    try_openbb("#12 PPP - USD/EUR Exchange Rate (EXUSEU)",
        lambda: obb.economy.fred_series(symbol="EXUSEU", limit=5))
...
if OPENBB_OK:
    ...
    try_openbb("#13 PCE Inflation",
        lambda: obb.economy.fred_series(symbol="PCEPI", limit=5))

    # Unemployment
    try_openbb("#14 Unemployment Rate",
        lambda: obb.economy.fred_series(symbol="UNRATE", limit=5))

    # NFP
    try_openbb("#14 Non-Farm Payrolls",
        lambda: obb.economy.fred_series(symbol="PAYEMS", limit=5))

    # MSTR
    try_openbb("#15 MSTR Price (Bitcoin Proxy)",
        lambda: obb.equity.price.historical(symbol="MSTR", limit=5))
...
if OPENBB_OK:
    ...
    try_openbb("#18 US 10Y Treasury Yield",
        lambda: obb.economy.fred_series(symbol="DGS10", limit=5))

    # 2Y yield
    try_openbb("#16 US 2Y Treasury Yield",
        lambda: obb.economy.fred_series(symbol="DGS2", limit=5))

    # Oil
    try_openbb("#17 Oil Price (WTI)",
        lambda: obb.economy.fred_series(symbol="DCOILWTICO", limit=5))

    # Copper
    # Copper via yfinance
    try_openbb("#17 Copper Price (HG=F)",
        lambda: obb.equity.price.historical(symbol="HG=F", limit=5))

    # Gold via yfinance
    try_openbb("#17 Gold Price (GC=F)",
        lambda: obb.equity.price.historical(symbol="GC=F", limit=5))
...
if OPENBB_OK:
    ...
    try_openbb("#19 Global PMI / CLI (OECD)",
        lambda: obb.economy.composite_leading_indicator(country="united_states", provider="oecd", limit=5))

# On-Chain via Blockchain.com
code, data = fetch_url("https://api.blockchain.info/charts/hash-rate?timespan=1days&format=json")
...
try:
    ...
    with urllib.request.urlopen(req, timeout=10) as r:
        size = len(r.read())
        record("#21 Geopolitical Risk Index (Iacoviello)", "PASS", f"{size} bytes downloaded"  )
except Exception as e:
