...
class SetupLearner:
    ...
    def recommend_setup_action(
        ...
        Currently returns neutral multipliers (1.0/1.0) so the learning
        policy collects training data without influencing trade sizing.
        Dynamic scaling can be re-enabled once the model is validated.
        """
        return {
            "setup": setup,
            "regime": regime,
            "should_trade": True,
            "reason": "learning policy neutral — data collection only",
            "predicted_pnl": 0.0,
            "win_probability": 0.5,
            "size_multiplier": 1.0,
