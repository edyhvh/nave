...
def journal_create():
    ...
    market_type = _select_option("Select market type", list(MARKET_TYPES), default="futures")
    trading_mode = _select_option("Select trading mode", list(TRADING_MODES), default="demo")
...
def run_trading(
    ...
    if strategy == "cot-weekly":
        ...
        if live:
            cmd.append("--live")
        typer.echo(f"Running {strategy} (paper=True)")
...
def cot_report(
    coins: str = typer.Option("BTC ETH", help="Coins to analyze (space-separated)"),
    ...
    def _parse_date(value: str) -> _date | None:
        dt = (
            _dt.fromisoformat(str(value))
            if "T" in str(value)
            else _dt.strptime(str(value), "%Y-%m-%d")
        )
        return dt.date()
            ...
            for coin in coin_list:
                ...
                if raw:
                    ...
                    if "market_and_exchange_names" in df.columns:
                        names = df["market_and_exchange_names"].astype(str).unique().tolist()
    ...
    for coin in coin_list:
        ...
        if not b:
            continue
        as_of_raw = b.metadata.get("as_of_date", b.metadata.get("report_date", "N/A"))
        as_of = None
        try:
    ...
    as_of_dates = [
        biases[c].metadata.get("as_of_date", biases[c].metadata.get("report_date", "N/A"))
        for c in coin_list
        if c in biases
    ]
    release_dates = [
        biases[c].metadata.get("release_date", "N/A") for c in coin_list if c in biases
    ]
    ...
    as_of_fof = next(
        (
            biases_futures_and_options[c].metadata.get(
                "as_of_date", biases_futures_and_options[c].metadata.get("report_date", "N/A")
            )
            for c in coin_list
    ...
    for coin in coin_list:
        ...
        vol = m["volatility"]
        direction = "LONG" if b.bias == "bullish" else ("SHORT" if b.bias == "bearish" else "FLAT")
        ...
        typer.echo(f"│ Market Regime (COT flow): {m['market_regime']} | Weekly Vol: {vol:.1%}")
        typer.echo("│")
        typer.echo(f"│ Net Non-Comm: {b.net_non_commercial:+,} ({net_label})")
        typer.echo(f"│ % of OI: {float(m['pct_oi']):.1f}% ({pct_side} position)")
        typer.echo(f"│ Weekly Δ Non-Comm: {b.weekly_change:+,}")
        typer.echo(f"│ Net Commercial: {b.net_commercial:+,}")
        typer.echo(f"│ Open Interest: {b.open_interest:,} (Δ {b.oi_change_pct:+.1f}%)")
        typer.echo("│")
        ...
        typer.echo(f"│ Historical Context: {m.get('percentile_interpretation', 'N/A')}")
        ...
        typer.echo("│")
        typer.echo(f"│ COT Interpretation: {m.get('cot_interpretation', 'N/A')}")
