...
| N5 | experiment/n5-squeeze-discovery | Volatility squeeze breakout as 4th bias source (BB<5%, ATR<2%, 7d squeeze → breakout bar → direction → downstream gates) | — (discovery only) | — | ✅ VIABLE — 6 TP / 0 FP (100% precision); avg expansion 29.2%; proposal ready for A/B test | — |
| N3b | experiment/n3-cross-asset-confirm | cross-asset soft confirmation (sub-threshold velocity + other-coin agreement) | +28.70 vs control +28.39 (v=0.5); sweep 0.3–1.0 all fail strict rule | +0.31 / WR −10.5pp | ❌ REJECT — added trades ~coin-flip; OOS blind spot still unreachable (see `10_n3_cross_asset_confirm.md`) | — |
