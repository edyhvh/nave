...
class TestSetupDiscovery:
    ...
    def synthetic_cot_data(self):
        ...
        dates = pd.date_range(start='2022-01-01', end='2025-03-31', freq='W-TUE')
        np.random.seed(42)
    ...
    def test_momentum_filter_impact(self, synthetic_cot_data):
        ...
        perf_no_momentum = self._simulate_forward_returns(signals_no_momentum, forward_days=14)
        ...
        perf_with_momentum = self._simulate_forward_returns(signals_with_momentum, forward_days=14)
    ...
    def test_low_oi_filter(self, synthetic_cot_data):
        ...
        low_oi_data.loc[::10, 'open_interest'] = 500  # Every 10th record has low OI
        ...
        print(f"Filtered out: {len(signals_all) - len(signals_filtered)} low-OI signals")
    ...
    def test_confidence_scoring(self, synthetic_cot_data):
        ...
        for bucket_name, bucket_signals in buckets.items():
            if bucket_signals:
                perf = self._simulate_forward_returns(bucket_signals, forward_days=14)
        ...
        if buckets['high'] and buckets['low']:
            high_perf = self._simulate_forward_returns(buckets['high'], forward_days=14)
            low_perf = self._simulate_forward_returns(buckets['low'], forward_days=14)
    ...
    def test_regime_dependence(self, synthetic_cot_data):
        ...
        for regime, regime_signals in regime_performance.items():
            if regime_signals:
                perf = self._simulate_forward_returns(regime_signals, forward_days=14)
    ...
    def _generate_signals_from_data(
        ...
        for _, row in data.iterrows():
            ...
            if net_pct > 0:
                ...
                confidence = min(0.9, abs(net_pct) / 50 + abs(change) / oi * 100)
    ...
    def _simulate_forward_returns(
        ...
        sharpe = avg_return / returns_series.std() if returns_series.std() > 0 else 0
