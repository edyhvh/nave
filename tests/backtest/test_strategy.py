...
class TestCotWeeklyStrategy:
    ...
    def test_market_regime_performance(self, mock_client, strategy_config):
        ...
        for regime, metrics in result.regime_metrics.items():
            print(f"  {regime}: {metrics.win_rate:.1%} win rate, {metrics.total_trades} trades")
    ...
    def test_correlation_and_diversification(self, mock_client, strategy_config):
        ...
        signals = [
            Signal(coin='BTC', direction=Direction.LONG, confidence=0.8, source='cot'),
            Signal(coin='ETH', direction=Direction.LONG, confidence=0.7, source='cot'),
        ]
...
class TestStrategyRobustness:
    ...
    def test_empty_signal_handling(self, mock_client):
        ...
        strategy = CotWeeklyStrategy(client=mock_client, capital_usd=10000)
    ...
    def test_conflicting_signals(self, mock_client):
        ...
        signals = [
            Signal(coin='BTC', direction=Direction.LONG, confidence=0.6, source='cot'),
            Signal(coin='BTC', direction=Direction.SHORT, confidence=0.4, source='macro'),
        ]
